  ...
  class Program1
  {
    static async Task<int> Main1(string[] args)
    {
      ...
      try
      {
        ...
        // after SaveFirstSlideAsPng(inputPptxPath);
...
- Keep each mapping text concise and human-readable.";
          ...
          await OpenAiHelper.SendImageAndJsonToResponsesApiAsync(pngPath, finalResponse, customInstruction);
        }
        else
        { 
...
{
    public static async Task SendImageAndJsonToResponsesApiAsync(string imagePath, string shapesJson, string instruction)
    {
